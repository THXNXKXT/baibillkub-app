...
export default function PublicDoc({ doc, cust, owner, items }: { doc: Doc; cust: Cust; owner: Owner; items: Item[] }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-100 via-emerald-50 to-white py-8 px-4">
      <motion.div
      ...
      >
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="text-sm">
            <p className="text-[#6e6e73] text-xs">ลูกค้า</p>
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.id} className="border-b border-neutral-100">
                  ...
                  <td className="py-2 text-right text-[#6e6e73] w-16">×{Number(it.qty)}</td>
          ...
          <div className="text-sm text-right space-y-1">
            <p className="text-[#6e6e73]">รวม {fmt(doc.subtotal)} บาท</p>
            {Number(doc.tax) > 0 && <p className="text-[#6e6e73]">ภาษี {fmt(doc.tax)} บาท</p>}
            <p className="text-xl font-bold text-emerald-600">{fmt(doc.total)} บาท</p>
          </div>

          {doc.notes && <p className="text-xs text-[#6e6e73] border-t border-neutral-100 pt-3">{doc.notes}</p>}

          {/* actions */}
          {done ? (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={spring} className="text-center text-sm font-medium text-emerald-600 py-2">
              {done}
            </motion.p>
          ) : doc.type === "quotation" && doc.status === "sent" ? (
            <div className="flex gap-2 pt-2">
              <button onClick={() => respond(true)} className="flex-1 rounded-lg bg-emerald-500 text-white py-2 text-sm font-medium">ตกลง</button>
              <button onClick={() => respond(false)} className="flex-1 rounded-lg border border-neutral-200 py-2 text-sm text-[#6e6e73]">ไม่ตกลง</button>
            </div>
            ...
            <div className="text-center space-y-3 pt-2">
              ...
              <p className="text-xs text-[#6e6e73]">สแกนจ่าย {fmt(doc.total)} บาท</p>
              ...
              <button onClick={pay} className="w-full rounded-lg bg-emerald-500 text-white py-2 text-sm font-medium">แจ้งชำระเงิน</button>
            </div>
          ) : doc.status === "paid" ? (
            <p className="text-center text-sm font-medium text-emerald-600 py-2">ชำระเรียบร้อยแล้ว</p>
          ) : null}
        </div>
      </motion.div>
      <p className="text-center text-xs text-[#6e6e73] mt-4">สร้างด้วย baibillkub</p>
    </div>
  );
}
