...
const STATUS_DOT: Record<string, string> = {
  ...
  sent: "bg-amber-400",
  paid: "bg-[var(--color-accent)]",
  accepted: "bg-[var(--color-accent)]",
  rejected: "bg-red-400",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/documents/new" className="btn-accent px-4 py-1.5 text-[13px] font-medium">+ สร้าง</Link>
      </div>

      <div className="flex gap-1 text-[13px]">
        <Link href="/documents" className={`px-3 py-1.5 ${!type ? "chip-active" : "chip"}`}>ทั้งหมด</Link>
        {Object.entries(TYPE_LABEL).map(([v, l]) => (
          <Link key={v} href={`/documents?type=${v}`} className={`px-3 py-1.5 ${type === v ? "chip-active" : "chip"}`}>{l}</Link>
        ))}
      </div>

      {docs.length === 0 ? (
        <div className="card px-6 py-10 text-center">
          <p className="text-[13px] text-[var(--color-muted)]">ยังไม่มีเอกสาร — สร้างฉบับแรกเลย</p>
        </div>
      ) : (
